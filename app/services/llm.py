...
GROQ_API_KEY = _clean_env_var(os.getenv("GROQ_API_KEY"))
...
FALLBACK_CHAIN = [
    ("gemini", PRIMARY_MODEL),              # 1st: user config
...
if GROQ_API_KEY and groq:
    try:
        ...
        print(f"[WARN] Error initialising Groq client: {e}")
else:
    print("[WARN] GROQ_API_KEY not found or groq module missing. Groq models will be skipped.")
...
def _call_gemini(model_name, prompt, stream=False):
    """
    Call a Gemini model. Returns the text response, or a generator if stream=True.
...
def _call_groq(model_name, prompt):
    """
    Call a Groq model. Returns the text response.
    Raises exception on any error.
    """
    if groq_client is None:
        raise ValueError("Groq client not initialised (missing API key or groq module).")
...
def _generate_with_fallback(prompt, task_name="answer", preferred_model=None):
    """
    Attempt to generate content across the full FALLBACK_CHAIN.
    If preferred_model is specified, try that first.
    """
    ...
    if preferred_model:
        ...
        pref_lower = preferred_model.lower()
        if any(x in pref_lower for x in ["llama", "mixtral", "gemma"]):
            pref_provider = "groq"
        elif "gpt" in pref_lower:
            pref_provider = "openai"
        elif "claude" in pref_lower:
            pref_provider = "anthropic"
            
        chain.insert(0, (pref_provider, preferred_model))
        ...
        for attempt in range(3):
            try:
                if provider == "gemini":
                    ...
                    text = _call_groq(model_name, prompt)
                else:
...
def generate_answer(question, context, return_metadata=False, model_id=None):
    """
    ...
    result = _generate_with_fallback(prompt=prompt, task_name="QA", preferred_model=model_id)
...
def generate_answer_stream(question, context):
    """
    Generate an answer using the RAG context, yielding tokens as they arrive.
    """
    prompt = _build_qa_prompt(question, context)
    
    # We will try the primary model, and if it fails immediately, fallback.
    # Due to the complexity of falling back mid-stream, we only catch connection errors.
    try:
        if gemini_client is None:
            yield "Gemini API key is not configured."
            return
            
        stream = _call_gemini(PRIMARY_MODEL, prompt, stream=True)
        for chunk in stream:
            if chunk.text:
                yield chunk.text
    except Exception as e:
